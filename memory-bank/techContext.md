...
直接操作する場合は `docker-compose -f docker-compose-dev.yml ...` を使用する。

## プロジェクト共有Skill

- `carbohydrate-dev-environment`: 開発環境操作一式
- `carbohydrate-production-deploy`: Amazon Lightsail本番デプロイ
- `carbohydrate-git-workflow`: Git操作。コミットメッセージは日本語必須

本番環境はLightsail上で完結し、意図的に `docker-compose-dev.yml` を使用する。
...
- Docker Desktop利用時は、Arch LinuxへのWSL integrationが無効だとDocker Composeを実行できない。
