...
本番ディスク不足、ログ保管、セキュリティレポート過多への対策を本番へ反映し、
運用資料を`docs/operations/production-maintenance.md`へ集約した。
...
1. S3の保存量と料金を定期確認する。自動削除はユーザー方針により設定しない。
2. ログ転送用IAMアクセスキーの最終使用日時を確認し、必要に応じてローテーションする。
3. ディスク使用率、journal容量、Docker資産、cron実行結果を運用資料に沿って定期確認する。
