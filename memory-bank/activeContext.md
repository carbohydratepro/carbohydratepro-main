...
家計簿へ選択月・先月・全期間平均の比較グラフを追加し、家計簿デモの検索・絞り込み・
年表示・一覧部分更新を実画面と同じように操作できるよう改善した。
...
- 現在の変更は未コミット。コミット・push・本番反映はこの依頼では実施していない。
...
- コミット`b978ac5`まで本番へ反映済み。初回転送は172オブジェクト・33,835,782バイトをS3で確認し、journalは4.0GBから72MB、ディスクは使用率24%・空き31GBになった。4コンテナ稼働とHTTP 200を確認した。
...
1. 家計簿の月次比較とデモ同期の未コミット差分を、ユーザー依頼があればコミット・push・本番反映する。
