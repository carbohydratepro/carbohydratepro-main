...
## 予算管理機能（新画面）
- **実装内容**: 家計簿に「予算」画面（`/carbohydratepro/budget/`）を新設。全体の月予算とカテゴリ別の月予算を設定し、当月の支出（Transaction）と突き合わせて消化状況を色分け進捗バー（緑=ok / 黄=80%以上warning / 赤=超過over）で表示する。`Budget` モデル（migration 0034、全体予算はユーザーごと1件・カテゴリ別は一意制約、全体予算未設定時はカテゴリ予算合計を目安に表示）、`build_budget_overview`（消化率・残額・ステータス算出）、`budget_view`（設定/更新/解除）を追加。ダッシュボードの収支カードに「今月の予算」バーと超過警告アイコン、家計簿ページに「予算」ボタン、デモ対応（`/demo/budget/`・色分けバー・超過アラート・フォーム送信ブロック）も実装。Djangoテスト18件・E2E 1件（E2E-BUDGET-001）追加、全スイート成功（Django 473件・E2E 69件）。
- **実装日時**: 2026-07-11 01:30

## UXバグまとめ修正（タイトル・全削除確認・削除a11y・デモ通知）
- **実装内容**: task.md の確定バグ4件をまとめて修正。①プロフィール編集/パスワード変更のページタイトルが「サインアップ」のままだった問題（共用テンプレート `user_form.html` に `page_title` を渡して修正）。②一時タスクの「全削除」「セット削除」の `window.confirm` 依存（ブラウザ固着の恐れ）を、`toast.ts` に追加したアプリ内確認ダイアログ `showConfirm` に置き換え。③一時タスクの削除オーバーレイを `<div>` から `aria-label` 付き `<button>` に変更（キーボード・支援技術・テストから操作可能に）。④デモの家計簿登録で意図的な reject が原因のエラーalert/コンソールエラーが出ていたのを抑制（サインアップモーダルのみ表示）。あわせて、UI刷新でモバイルの固定フッター被りが解消済み、本番は `DEBUG=False` で404が内部情報を出さないことを確認し、task.md の該当2項目も整理。Djangoテスト3件・E2E 3件（E2E-UNDO-003/004、E2E-DEMO-002）追加。
- **実装日時**: 2026-07-10 06:00

## 削除のUndoトーストと選択モード一括削除
- **実装内容**: 共通トースト部品 `src/ts/toast.ts`（`showToast` / Undo付き `showUndoToast`）を追加し、全画面で読み込み。①一時タスク（カンバン）の削除を「即削除＋Undoで再作成」に変更し、スマホのドラッグ／長押しでの誤削除を数秒間「元に戻す」で復元できるようにした（keepalive で離脱時も確定、モバイルのドラッグ削除ゾーンを小さめ＋ハザード縞にして誤タップ抑制）。②メモ・買い物・家計簿の一覧に選択モード＋一括削除を追加。共通モジュール `src/ts/bulk_delete.ts` と DOM 契約（`data-bulk-container`/`data-bulk-url`/`bulk-item`/`data-bulk-id`/`.bulk-check`）で横展開し、下部バーに選択件数と削除ボタン、削除は同じ Undo トーストで取り消し可能。バックエンドは共通ヘルパー `app/bulk_delete.py`（所有者チェック）で各エンドポイントを追加。デモモードは実削除せずサインアップモーダルでブロック。Djangoテスト10件・E2E 3件（E2E-UNDO-001/002、E2E-BULK-001）追加、全スイート成功（Django 451件・E2E 65件）。
- **実装日時**: 2026-07-09 19:30

## 外部カレンダーの取り込み（ICS購読）
- **実装内容**: Googleカレンダーの「非公開のiCal URL」やiCloudのパブリックカレンダーURL（webcal://対応）を登録し、外部の予定を読み取り専用で取り込む機能。`ExternalCalendar`/`ExternalEvent` モデル（マイグレーション0033）、icalendar + recurring-ical-events によるRRULE展開（過去30日〜未来370日を洗い替え）、SSRF対策（プライベートIP拒否・リダイレクト再検証・5MB上限）、cronによる30分ごとの自動同期と追加時の即時同期。タスク設定画面に追加・再同期・削除UI、月カレンダー・日表示ガント・日別モーダル・ダッシュボードにカレンダー色で表示（編集不可）。デモモード対応。テスト18件追加、実Googleカレンダー（日本の祝日28件）で同期・表示を確認。
- **実装日時**: 2026-07-09 05:00

## UI刷新（サイドバー+ボトムタブ）とスケルトンスクリーン
- **実装内容**: ナビゲーションをデスクトップは左サイドバー、モバイルは下部タブ（ホーム/家計簿/予定/習慣/その他）のアプリシェルに刷新。ヘッダー帯は従来のダークカラー（#343a40）を維持し、デザイントークンを藍（#33518e）アクセント+寒色ニュートラルに変更、Bootstrap primary系クラスもトークンに追従。ログイン済みの認証系画面も同じシェルに統一し、デモモードにも対応。あわせて全画面共通のスケルトンスクリーン（`skeleton.ts`）を追加し、画面遷移を伴うクリック/フォーム送信を検知した瞬間にコンテンツ領域をshimmer付きスケルトンで覆う（preventDefault済み操作・外部リンク・ページ内アンカーは対象外、bfcache復元時は自動解除、prefers-reduced-motion対応）。Djangoテスト7件・E2E 2件（E2E-RESP-002, E2E-UI-001）追加、全スイート成功（Django 425件・E2E 62件）。
- **実装日時**: 2026-07-08 11:30
