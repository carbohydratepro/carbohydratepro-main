...
function initializeTimeSelects() {
    ['id_start_time', 'id_end_time'].forEach(inputId => {
        ...
        if (!inputEl || inputEl.dataset.timePickerInitialized)
            return;
        const input = inputEl;
        const currentValue = input.value;
        const parts = currentValue ? currentValue.split(':') : [];
        const currentHour = (_a = parts[0]) !== null && _a !== void 0 ? _a : '';
        const currentMinute = (_b = parts[1]) !== null && _b !== void 0 ? _b : '';
        ...
        for (let h = 0; h < 24; h++) {
            ...
            if (currentHour !== '' && parseInt(currentHour) === h)
                opt.selected = true;
        ...
        for (let m = 0; m < 60; m += 5) {
            ...
            if (currentMinute !== '' && parseInt(currentMinute) === m)
                opt.selected = true;
...
function setTimePickerValue(inputId, timeStr) {
    const hiddenInput = document.getElementById(inputId);
    if (!hiddenInput)
        return;
    hiddenInput.value = timeStr;
    const wrapper = hiddenInput.closest('.time-picker-wrapper');
    if (!wrapper)
        return;
    const [hourStr = '', minuteStr = ''] = timeStr ? timeStr.split(':') : [];
    const hourSelect = wrapper.querySelector('.time-hour-select');
    const minuteSelect = wrapper.querySelector('.time-minute-select');
    if (hourSelect)
...
function setDefaultTimes() {
    const startHidden = document.getElementById('id_start_time');
    const endHidden = document.getElementById('id_end_time');
...
function toggleTimeFields() {
    ...
    if (allDayCheckbox && timeFields.length > 0) {
        ...
        timeFields.forEach(field => {
            ...
            if (isAllDay) {
                // 時・分セレクトをクリア
                field.querySelectorAll('.time-hour-select, .time-minute-select').forEach(sel => {
                    sel.value = '';
                });
                // 隠しフィールドもクリア
                field.querySelectorAll('input[data-time-picker-initialized]').forEach(inp => {
                    inp.value = '';
                });
            }
        });
