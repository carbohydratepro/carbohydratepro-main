...
本番ディスク不足とセキュリティレポート過多を調査し、Docker保守と通知方式を改善している。
...
- 1分ごとのsecurity.log通知と5分ごとのWARNING通知は過剰だったため、本番では一時停止。コードは毎日6時に過去24時間を1通へ集約し、通常のbot探索・CSRF等は件数のみ、特権ログインとERROR/CRITICALだけ詳細通知する方式へ変更した。
- system journalは約4GB。通常S3の専用バケット `carbohydratepro-system-logs` へ、system journalとアプリログを毎時圧縮転送する。S3の容量・SHA-256・暗号化を検証できた場合だけ、ローテーションログの削除とjournalの7日／1GB整理を行う。失敗時は削除せず、日次レポートへ最大1日1回通知する。S3側の自動削除は設定しない。
...
1. Docker保守・日次通知の運用改善をコミット・pushし、本番リリースを再実行する。
2. 本番のmigration 0035・0036・0037、静的ファイル、ヘルスチェック、容量、cronを確認する。
3. 専用S3バケットと本番ロール権限を用意し、system journal・アプリログの圧縮退避と30日TTLを実装する。
