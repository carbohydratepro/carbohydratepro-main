...
2026-09-09、全画面UI統一を実装。共通部品CSSを通常・認証ベースへ追加し、問い合わせ・タブ・料理・マイページ・管理・障害案内まで調整。Djangoシステム管理も専用テーマを適用。非遮断通知と確認ダイアログのフォーカスを補強。Django535件、Playwright重複除外50ケース成功。適用範囲・検証境界は `docs/ui-redesign.md` の「全画面のUI統一」。コミット・push・本番操作なし。

2026-09-09、承認済みのコンパクトなボタン・役割別スタイルを本文とモーダルへ共通適用。買い物・一時タスクの独自寸法と予定カードの余白も整理。既存Playwright46件成功、共通寸法のPC/タッチ回帰テストを追加。詳細は `docs/ui-redesign.md`。開発環境のみで、コミット・push・本番反映なし。

2026-09-08、ユーザーのモデル変更後の再調査依頼に基づきUI刷新を再評価した。
汎用containerの一律拡幅を用途別に限定し、非表示グラフの遅延生成・逐次アニメーション廃止・操作色統一を追加。詳細と残件は `docs/ui-redesign.md`。本番操作・コミット・pushなし。

前件のローカル認証ZAP Spiderによる誤更新・削除からの開発DB復旧は完了。本番環境は対象外で影響なし。今後、認証済み画面とDjango管理サイトではSpiderを使用しない。

## ZAP誤更新・削除の復旧（2026-09-07）

- 停止状態の物理DBスナップショットと保持WALのフルページイメージから、スキャン前のカテゴリ46件、削除された取引166件、定期支払9件を全カラム復号した。
- Spiderにメールアドレスを変更されcleanupから漏れた通常ユーザー用ZAP一時アカウントは、Django ORMで関連23オブジェクトとともに削除した。
- 検証済み復旧SQLを元の開発DBへ適用し、カテゴリ46、取引438、定期支払9、予算4へ復旧した。ZAP一時ユーザー・`ZAP`名カテゴリ・孤立外部キーはいずれも0件。
- WAL復号元と元DBの全行・全カラムハッシュは一致し、Django `check`は問題0件、デモ家計簿はHTTP 200だった。
- 復旧直後の論理ダンプと調査資料はGit管理外の`workspace/zap/2026-09-01/recovery/`へ権限600で保存した。
- db・gunicorn・nginx・cronは再起動済み。調査用DBコンテナ2台はボリュームを保持して停止中。

## スマートフォン表示改善（2026-08-30）

- 全画面のスマートフォン表示を整理し、共通余白・固定ナビ安全領域・フォーム・表・モーダルを320〜430px幅へ対応した。
- 家計簿グラフはスマホで個別に折りたたみ、スケジュール月表示は予定名の代わりに色印と件数を表示する。

- 767.98px以下の共通コンテナを左右12pxへ統一し、見出し、カード、フォーム、タッチ領域、横スクロール表、モーダル、トップバー、下部ナビのsafe-areaを整理した。
- 家計簿、スケジュール、メモ、買い物、一時タスクは主要操作を常時表示し、選択・設定などをスマホの「その他」メニューへ移した。デスクトップでは従来同様に全操作を表示する。
- 家計簿はカテゴリグラフのみ初期展開し、費用タイプ、日別支出、日別収支を個別に開閉する。Chart.jsは折りたたみ展開時に幅を再計算する。
- スケジュール月表示は7列を維持し、スマホでは日付、予定色最大3個、件数だけを表示する。日付セルの既存一覧モーダルと日表示ガントは維持する。
- ホーム、習慣、メモ、買い物、一時タスク、料理記録、認証、設定、問い合わせ、運営管理のカード・入力・表・管理グラフを共通ルールでコンパクト化した。
- `E2E-RESP-003`〜`005`を追加し、320px、390px、430pxの横はみ出し、下部ナビ被り、グラフ開閉、月カレンダー操作を検証対象にした。
- `npm run check`は成功。本番デモHTMLへCSSだけを一時適用した390px確認では主要7画面すべて横はみ出し0px。Dockerデーモン停止中のためDjangoチェックとローカルPlaywright実行は保留。
...
### 2026-09-09 メモ画面の操作試作

- フィードバック後、ボタンを36px/タッチ40px・角丸8pxへ縮小し、カード内16〜18pxの余白を確保。お気に入り/その他/PCツールバー/モーダル補助ボタンのスタイルを整理。全画面への展開はまだ行わない。

- ユーザーが操作現代化案の「1画面で試作」を承認。メモ一覧に44pxボタンと「…」編集/削除、保存中状態・入力欄エラー・成功トーストを導入。お気に入りのみ通常一覧で非リロード化。保存/削除後の再読み込みは維持。
- ライブラリ追加/Bootstrap移行/全画面展開は未実施。詳細は `docs/ui-redesign.md`。本番操作なし。
- npm run check・Djangoメモ31件・Playwright16件成功。390pxの操作メニュー目視確認済み。試作用一時ユーザーの後始末完了。開発環境へ反映済み。

### 2026-09-09 文字の読みやすさの微調整

- ユーザー希望により基準文字サイズを16.5px相当へ微増、ボタン/入力ラベルを500に調整。既存の見出しの太字と色は維持。開発環境へ反映済み。
- 型チェック/ビルド・関連Playwright29件・差分チェック成功。320/390/430px確認と390pxホーム/ログインの目視確認済み。一時テストユーザーの削除も完了。コミット・push・本番反映なし。

### 2026-09-08 後続UIフェーズと再開時の状態

- ユーザーはメモ厳守より使いやすさ・デザイン・応答性を優先し、適宜変更を許可。家計簿・習慣・設定・料理・認証フォームと配信/キャッシュを改善済み。詳細は `docs/ui-redesign.md`。
- Docker起動後、停止中の開発サービスを起動。最終変更を含め `npm run check`、collectstatic、Gunicorn HUP、Nginx構文検証が成功。localhost:8000で確認可能。
- 最終再実行でDjango全534件・Playwright対象45件がすべて成功（E2E 58.6秒）。設定編集モーダル、スマホ幅、圧縮/キャッシュ/オフラインを含む。一時ユーザーと関連データの削除も完了。
- テスト用ユーザーは `scripts/test-ui-session.mjs` が一意に作成しfinallyで削除する。対象specを引数に渡すローカル専用処理。既存ユーザーのリセット・管理者化はしない。
- ロゴをAIで描き直さず通常のリサイズ/圧縮で軽量化する方針を非同期で確認中。返答未確認のため元画像は未変更。コミット/push/本番反映の現在の依頼はない。

### 2026-09-07 UI刷新の第1段階

- 共通デザイン、ホームの補助情報折りたたみ、PCカレンダー幅、機能CSS先行読み込み、jQuery圧縮版を実装。詳細と残件は `docs/ui-redesign.md`。
- ローカルの静的ファイル収集後にGunicornへHUPを送り、Manifest保持の古い参照を更新して検証した。本番操作なし。
- `npm run check`、Django check、デモ・レスポンシブ・UIのPlaywright 11件成功。認証必須CRUDは未実行。コミット・push・本番反映なし。
