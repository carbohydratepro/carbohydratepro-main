...
家計簿へ選択月・先月・全期間平均の比較グラフを追加し、家計簿デモを実画面へ同期した。
本番のセキュリティレポート頻回通知を調査し、日付単位の送信重複防止を追加した。

## セキュリティレポート重複防止（2026-08-24）

- 本番Lightsail `docker-django-app-carbohydratepro`（54.238.169.177）を読み取り調査した。調査時点のコミットは`b978ac5`で、ホストのroot/ec2-user crontabに監視ジョブはなく、cronコンテナは1個だけ稼働していた。
- コンテナ内の設定は毎日6:00 JSTの`check_security_log --hours 24`のみで、`SEND_INSTANT_SECURITY_EMAIL=False`。直近cronログ、アプリログ、空のメールキューにも1分間隔の送信痕跡はなかった。
- 同一AWSアカウントの東京リージョンには稼働Lightsailが上記1台だけで、稼働EC2もなかった。ローカルDockerにも別の旧cronコンテナは存在しなかった。このため現行本番が調査時刻以降の1分通知元とは認められず、継続時の特定には受信メールの件名・本文末尾・Receivedヘッダーが必要。
- `SecurityReportDelivery`（migration 0007）へJST日付ごとの送信状態を保存し、DB行ロックにより誤った毎分実行・重複cron・同時実行でも成功済みの日次レポートを最大1日1通に制限した。SMTP失敗時は同日再試行できる。
- 家計簿変更は`5f1ca5e`、通知対策は`b60a5ad`として分離コミットした。本番反映対象はこの2コミット。
- 検証: ログ監視5件、関連123件、Django全534件、migration適用と未生成差分なしを確認した。

## 家計簿の月次比較とデモ同期（2026-08-24）

- 月表示に「先月・全期間平均と比較」トグルを追加し、収入・支出・収支を3系列の棒グラフで比較できるようにした。開閉状態はブラウザーに保存する。
- 全期間平均は、ログインユーザーのうち現在の検索・取引種別・費用タイプ・カテゴリ・支払方法条件に一致する「記録のある月」だけを分母にする。日付一点の条件は月単位比較には適用しない。
- デモ家計簿を3か月分のデータへ拡張し、検索、複数の含む／除外条件、並べ替え、表示件数、月／年切替、年次グラフを実際に動作させた。
- デモのグラフ操作も実画面と同じ一覧部分HTMLを返し、画面とグラフを再読み込みせず取引一覧だけを更新する。
- 検証: 家計簿・デモ関連Django 118件、`npm run check`、デモPlaywright 3件（`E2E-DEMO-001`〜`003`）が成功した。ホストにRuff/Pyrightがないため両ツールは未実行。
- 変更は`5f1ca5e`としてコミット済み。

## 画面改善（2026-08-23）

- 家計簿サマリーの収入・支出・収支を中央揃えにし、一覧行の金額は従来どおり右揃えを維持した。
- 家計簿グラフのクリック／タップは、ページ遷移せず取引一覧の部分HTMLだけを非同期取得して差し替える。グラフとサマリーは再描画せず、URLと詳細絞り込みフォームだけを同期する。
- 連続操作時は古い一覧取得を中断し、取得失敗時は現在表示を維持してエラートーストを出す。一覧差し替え後も長押し削除とダブルクリック編集を再初期化する。
- タスク設定の週開始曜日をBootstrapのボタントグル依存から、ラベル付きのネイティブラジオ入力へ変更し、月曜開始を選択できるようにした。
- 検証: 機能回帰14件、家計簿・タスク既存173件、Django全527件、`npm run check`、`E2E-FIXES-001` が成功した。

## 本番運用改善（2026-08-23更新）

- 40GBディスクが98%（空き約1GB）になった原因は、過去ビルドの未使用イメージとBuildKitキャッシュだった。7日より古い未使用資産を削除し、使用率33%・空き約27GBまで回復した。
- 稼働イメージは約1.5GBで急肥大はなく、72イメージ中4個のみ稼働、BuildKitの古いレイヤーが主因だった。デプロイ前と毎週日曜4:30（JST）に、7日より古い未使用イメージ・キャッシュを自動削除する処理を追加した。
- 本番ホストのroot/ec2-user双方に残っていた、存在しない旧監視スクリプトのcronをバックアップ後に削除した。
- 1分ごとのsecurity.log通知と5分ごとのWARNING通知を廃止した。本番では毎日6時に過去24時間を最大1通へ集約し、通常のbot探索・CSRF等は件数のみ、特権ログインとERROR/CRITICALだけ詳細通知する。
- 通常S3の専用バケット `carbohydratepro-system-logs` へ、system journalとアプリログを毎時圧縮転送する。S3の容量・SHA-256・暗号化を検証できた場合だけ、ローテーションログの削除とjournalの7日／1GB整理を行う。失敗時は削除せず、日次レポートへ最大1日1回通知する。S3側の自動削除は設定しない。
- LightsailインスタンスとS3バケットの所有アカウントは `273296317958`。インスタンス内STSが示す `851725228159:assumed-role/AmazonLightsailInstanceRole` はLightsail管理セッションであり、顧客側IAMから編集できない。通常S3への接続には専用IAMユーザー `carbohydratepro-system-log-uploader` を使い、キーは本番のroot専用設定に保存した。
- 2026-08-24の通知調査開始時点ではコミット`b978ac5`まで本番へ反映済み。初回転送は172オブジェクト・33,835,782バイトをS3で確認し、journalは4.0GBから72MB、ディスクは使用率24%・空き31GBになった。4コンテナ稼働とHTTP 200を確認した。
...
- 検証: PostgreSQL開発DBへmigration 0037を適用し、Django全521件、`npm run check`、`E2E-FIXES-001` が成功した。
...
- `E2E-COOKING-001` とDjangoテストを追加。PostgreSQL開発DBへmigration 0036を適用し、料理記録を含むDjango全521件、`npm run check`、`E2E-COOKING-001` が成功した。
...
1. 本番反映後も1分通知が継続する場合、受信メールの件名・本文末尾・Receivedヘッダーから現行／旧コマンドと送信元ホストを特定する。
2. S3の保存量と料金を定期確認する。自動削除はユーザー方針により設定しない。
3. ログ転送用IAMアクセスキーの最終使用日時を確認し、必要に応じてローテーションする。
4. ディスク使用率、journal容量、Docker資産、cron実行結果を運用資料に沿って定期確認する。
