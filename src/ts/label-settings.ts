...
// タブ切り替え
function switchColorTab(tabName: string, modalId: string): void {
    const modal = document.getElementById(modalId);
    if (!modal) return;
    modal.querySelectorAll<HTMLElement>('.color-picker-tab').forEach(tab => {
        tab.classList.toggle('active', tab.dataset.tab === tabName);
    });
    modal.querySelectorAll<HTMLElement>('.color-picker-content').forEach(content => {
        content.classList.toggle('active', content.dataset.tab === tabName);
    });
}

// カラーテンプレートの選択
function selectColorTemplate(color: string, modalId: string): void {
    const modal = document.getElementById(modalId);
    if (!modal) return;
    const colorInput = modal.querySelector<HTMLInputElement>('input[name="color"]');
    const colorPreview = modal.querySelector<HTMLElement>('.color-preview');

    // 選択状態を更新
    modal.querySelectorAll<HTMLElement>('.color-template-item').forEach(item => {
...
// カスタムカラーピッカーの変更
function updateCustomColor(input: HTMLInputElement, modalId: string): void {
    const modal = document.getElementById(modalId);
    if (!modal) return;
    const colorPreview = modal.querySelector<HTMLElement>('.color-preview');
...
// カラーテンプレートグリッドを生成
function generateColorTemplateGrid(containerId: string, modalId: string): void {
...
// モーダルが開かれたときの初期化
function initializeColorPicker(modalId: string): void {
    const modal = document.getElementById(modalId);
    if (!modal) return;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll<HTMLElement>('[id^="editLabelModal"]').forEach(modal => {
    ...
    document.querySelectorAll<HTMLElement>('[id^="editMemoTypeModal"]').forEach(modal => {
...
function copyCalendarFeedUrl(): void {
    const input = document.getElementById('calendar-feed-url') as HTMLInputElement | null;
