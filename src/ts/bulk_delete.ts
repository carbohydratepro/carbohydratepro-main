...
(() => {
    ...
    function initBulkContainer(container: HTMLElement): void {
        const url = container.dataset['bulkUrl'];
        const toggle = container.querySelector<HTMLElement>('[data-bulk-toggle]');
        ...
        const countEl = bar.querySelector<HTMLElement>('.bulk-action-count');
        const deleteBtn = bar.querySelector<HTMLButtonElement>('.bulk-action-delete');
        const cancelBtn = bar.querySelector<HTMLButtonElement>('.bulk-action-cancel');

        const items = (): HTMLElement[] =>
            Array.from(container.querySelectorAll<HTMLElement>('.bulk-item'));

        function updateBar(): void {
            if (countEl) countEl.textContent = `${selected.size}件を選択中`;
        ...
        function setChecked(item: HTMLElement, checked: boolean): void {
            ...
            if (!id) return;
            const box = item.querySelector<HTMLInputElement>('.bulk-check');
        ...
        toggle.addEventListener('click', () => {
            if (container.classList.contains('bulk-mode')) exitMode();
            else enterMode();
        });
        ...
        container.addEventListener('click', (e: MouseEvent) => {
            if (!container.classList.contains('bulk-mode')) return;
            const target = e.target as HTMLElement;
            const item = target.closest<HTMLElement>('.bulk-item');
            ...
            e.stopPropagation();
            const id = item.dataset['bulkId'];
            if (id) setChecked(item, !selected.has(id));
        }, true);
        ...
        deleteBtn?.addEventListener('click', () => {
            ...
            if ((window as unknown as { DEMO_MODE?: boolean }).DEMO_MODE) {
                void fetch(url, {
                    method: 'POST',
    ...
    document.addEventListener('DOMContentLoaded', () => {
        document.querySelectorAll<HTMLElement>('[data-bulk-container]').forEach(initBulkContainer);
    });
})();
