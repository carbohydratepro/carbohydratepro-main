...
class ListViewTests(TestCase):
    def setUp(self):
        ...
        self.user = self.User.objects.create_user(
            email="user@example.com", username="tester", password="pass12345", is_email_verified=True
        )
        self.client.login(username=self.user.email, password="pass12345")

    # 家計簿
    def _create_expense_data(self, count=25):
    ...
    def test_expenses_default_pagination_20(self):
    ...
    def test_expenses_per_page_10(self):
    ...
    def test_expenses_filter_by_transaction_type(self):
        pay = PaymentMethod.objects.create(user=self.user, name="カード")
        cat = Category.objects.create(user=self.user, name="食費")
        Transaction.objects.create(
            user=self.user,
    ...
    # メモ
    def _ensure_memo_type(self):
        memo_type, _ = MemoType.objects.get_or_create(user=None, name="メモ", defaults={"color": "#007bff"})
        return memo_type

    def test_memo_list_pagination(self):
    ...
    def test_memo_list_per_page_50(self):
        memo_type = self._ensure_memo_type()
        Memo.objects.bulk_create(
            [
                Memo(user=self.user, title=f"メモ{i}", memo_type=memo_type, content="内容")
    ...
    def test_memo_filter_by_type_and_favorite(self):
        memo_type = self._ensure_memo_type()
        idea_type, _ = MemoType.objects.get_or_create(user=None, name="アイデア", defaults={"color": "#28a745"})
        Memo.objects.create(user=self.user, title="メモ1", memo_type=memo_type, content="A", is_favorite=True)
        Memo.objects.create(user=self.user, title="アイデア1", memo_type=idea_type, content="B", is_favorite=False)
    ...
    # 買いものリスト
    def test_shopping_list_groups_items(self):
        ShoppingItem.objects.create(
            user=self.user, title="牛乳", frequency="one_time", remaining_count=0, threshold_count=1
        )
        ShoppingItem.objects.create(
            user=self.user, title="米", frequency="recurring", remaining_count=5, threshold_count=2
        )
    ...
    def test_shopping_update_counts(self):
        item = ShoppingItem.objects.create(
            user=self.user, title="水", frequency="one_time", remaining_count=1, threshold_count=0
        )
        resp = self.client.post(
            reverse("update_shopping_count", args=[item.id]),
    ...
    # スケジュール（タスク）
    def _create_tasks(self, count=25):
    ...
    def test_task_list_month_pagination_default(self):
    ...
    def test_task_list_day_pagination_10(self):
        self._create_tasks(12)
        today_str = timezone.now().date().strftime("%Y-%m-%d")
        resp = self.client.get(reverse("task_list") + f"?view_mode=day&target_date={today_str}&per_page=10")
        page = resp.context["tasks_page"]
        self.assertEqual(page.paginator.per_page, 10)
        self.assertEqual(len(page.object_list), 10)

    def test_task_week_start_session_default(self):
